...
object NoeonAiEngine {
    ...
    fun buildSystemPrompt(candidateExperiences: List<MemoryItem>): String = buildString {
        ...
        append("- EXTRACTED_REMINDERS are ONLY for explicit FUTURE tasks, events, meetings, appointments, extra classes, doctor visits, pickups, deadlines, or scheduled commitments that the user states they must perform or attend in the FUTURE (e.g. 'I have extra class on Sunday 2pm', 'meet doctor next Friday at 10am', 'remind me to pick up dad tomorrow').\n")
        append("- NEVER extract reminders for:\n")
        append("  1. Questions or inquiries asked by the user (e.g. 'What is my name?', 'Where do I live?', 'What is my assignment deadline?').\n")
        append("  2. Existing background facts or past details (e.g. 'My name is Dinesh', 'I am an engineer').\n")
        append("  3. General chit-chat or greetings.\n")
        append("- If no explicit FUTURE task, meeting, class, appointment, pickup, or reminder commitment exists in the user's message, you MUST output:\n")
